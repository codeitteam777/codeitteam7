...
| 이름 | 역할 | Github |
| :--- | :--- | :--- |
| **이승완(팀장, PM)** | 초기 베이스라인 코드, 의사 레이블링, EDA, 모델링 (Optimizer) | |
...
* **신규 목표:** `mAP 0.99` 달성을 기본값으로 간주. **"mAP 0.99를 달성하는 최소 비용(Cost)을 탐색"**하고 **"가장 낮은 Validation Loss(최고의 안정성)"**를 확보하는 것을 핵심 목표로 설정.
...
* **목표:** mAP 0.99 달성을 전제로, **모든 Validation Loss 항목(Box, Cls, DFL)에서 안정적인 성능**을 보이는 옵티마이저를 선정.
* **설정:** yolov8n, epochs=70, imgsz=640, batch=16, augment=False, lr0=0.001
* **결과:** AdamW가 모든 Loss 항목에서 가장 낮은 값을 기록하며 가장 안정적인 성능을 보여줌.

| 옵티마이저 | mAP@75 | Val Box Loss | Val Cls Loss | Val DFL Loss | 총 시간 |
| :--- | :---: | :---: | :---: | :---: | :---: |
...
* **Loss 항목별 의미:**
  - **Box Loss:** 바운딩 박스의 위치와 크기 예측 정확도 (낮을수록 정확한 위치 검출)
  - **Cls Loss:** 알약 클래스 분류 정확도 (낮을수록 정확한 약품 식별)
  - **DFL Loss:** 분포 포커싱 손실 - 객체 경계 예측 정확도 (낮을수록 정확한 경계 검출)

* **결론:** `Nadam`이 속도는 가장 빨랐으나, **`AdamW`**가 **모든 Val Loss 항목에서 최저값**을 기록하며 가장 안정적인 성능을 보여줌. 특히 **Cls Loss에서 가장 큰 격차**로 우수한 분류 성능을 보임. **`AdamW`를 Baseline 옵티마이저로 채택.**
...
* **팀 기준 Baseline:**
  ...
  - **Loss Targets:** 
    - `Val Box Loss: 0.1543` (이보다 낮춰야 함)
    - `Val Cls Loss: 0.1454` (이보다 낮춰야 함) 
    - `Val DFL Loss: 0.7621` (이보다 낮춰야 함)

### Phase 2: 비용 절감 및 Val Loss 최소화 (진행중)
* **공통 목표:** `yolov8n + AdamW + 70 Epochs` Baseline을 기준으로, **각 Val Loss 항목(Box, Cls, DFL)을 개별적으로 개선**하거나, 동일/유사 Loss를 더 적은 Epoch로 달성하는 설정 탐색.
* **이경식 (모델 크기):** (완료) `yolov8n`을 Baseline 모델로 채택.
* **오현민 (ImgSize / LR):** `ImgSize` (320, 640, 1280) 비교 / `AdamW` 기준 최적 LR 탐색.
* **최준영 (증강):** `Augment=True` (기본값) 설정이 `Augment=False` 대비 **각 Val Loss 항목 개선**에 효과가 있는지 비교.
* **박병호 (EDA):** `train/val/test` 세트 간 클래스 분포, BBox 면적 분포 비교 시각화. (3개 세트가 사실상 동일함을 증명)

### Phase 3: 최종 검증 (Test 셋)
* **목표:** `7:1.5:1.5` 분할의 유의미성 검증.
* **실행:** Phase 2에서 **"모든 Val Loss 항목이 가장 낮았던"** 모델 (`best.pt`)을 최종 선정.
* 해당 모델로 `Test` 셋(15%)의 mAP와 **Test Loss**를 측정.
* **예상 결론:** "EDA 및 Test 셋 검증 결과, Train/Val/Test 셋이 통계적으로 동일함을 최종 확인. 7:1.5:1.5 분할은 일반화 검증에 유의미하지 않음."

## 4. 다음 단계 (Next Steps)

- [✓] **Baseline 확정 (PM):** `yolov8n`, `AdamW`, `70 Epochs`, `Val Box Loss 0.1543`, `Val Cls Loss 0.1454`, `Val DFL Loss 0.7621`
- [ ] **오현민, 최준영:** Phase 2 실험 시작. 70 Epoch Baseline 기준으로 ImgSize, Augmentation 실험 진행 및 **각 Val Loss 항목 결과** 공유.
...
**참고: Loss 평가 기준**
- 모든 Loss 항목을 개별적으로 비교하고 평가
- Box Loss: 위치 검출 정확도
- Cls Loss: 분류 정확도  
- DFL Loss: 경계 검출 정확도
- 세 항목 모두 중요하며 균형있는 개선이 필요
